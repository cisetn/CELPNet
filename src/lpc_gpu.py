...
def lpc_analysis(wav, fl, fs, order, emph_coef=0.97, flag_emph=False):
    ...
    # frame_wined = windowing(framing(wav_tmp[:, 0], fl, fs))
    frame_wined = framing(wav_tmp[:, 0], fl, fs)
    ...
    err_signal = overlapadd(framed_err, fl, fs)
...
def lpc_synthesis(lpc, framed_resid, gain, hop_size):
    ...
    x = overlapadd_gpu(framed_x[:, :_f, lpc_order:], frame_size, hop_size)
...
def overlapadd(framed_x, frame_size, hop_size):
    ...
    for frame_idx in range(framed_x.shape[0]):
        ...
        x[wav_idx:wav_idx + frame_size] += framed_x[frame_idx, :]
...
def overlapadd_gpu(framed_x, frame_size, hop_size):
...
def main():
    ...
    resid = torch.from_numpy(resid).float().unsqueeze(0).unsqueeze(0)
    ...
    gain = torch.from_numpy(gain).float().unsqueeze(0).cuda()
    _lpc = _lpc.unsqueeze(0).cuda()
    ...
    # -> numpy
    _wav = _wav.to('cpu').detach().numpy().copy().squeeze()
